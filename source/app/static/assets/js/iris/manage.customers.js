...
function loadSheetJS(callback) {
    ...
    script.src = '/static/assets/js/plugin/xlsx/xlsx.full.min.js';
    script.onload = callback;
    document.head.appendChild(script);
}
function exportToExcel(customersData, staatsverwaltungData, hochschuleData, binnenmarktData) {
    const ws1 = XLSX.utils.json_to_sheet(customersData.map(entry => ({
        'Organisation': entry.org_name,
        'Kurzbezeichnung': entry.org_short,
        'Ressort': entry.org_top === "Staatsverwaltung" ? entry.org_name : entry.org_top,
        'Notiz': entry.contact_note,
        'E-Mail': entry.contact_email,
        'Telefon': entry.contact_work_phone,
        'Mobil': entry.contact_mobile_phone,
        'Rolle': entry.contact_role,
        'Name': entry.contact_name,
        'Notizen': entry.contact_note
    })));

    const ws2 = XLSX.utils.json_to_sheet(staatsverwaltungData.map(entry => ({
        'Organisation': entry.org_name,
        'Kurzbezeichnung': entry.org_short,
        'Top-Organisation': entry.org_top,
        'Notiz': entry.contact_note,
        'E-Mail': entry.contact_email,
        'Telefon': entry.contact_work_phone,
        'Mobil': entry.contact_mobile_phone,
    ...
    const ws3 = XLSX.utils.json_to_sheet(hochschuleData.map(entry => ({
        'Organisation': entry.org_name,
        'Kurzbezeichnung': entry.org_short,
        'Top-Organisation': entry.org_top,
        'Notiz': entry.contact_note,
    ...
    const ws4 = XLSX.utils.json_to_sheet(binnenmarktData.map(entry => ({
        'Organisation': entry.org_name,
        'Kurzbezeichnung': entry.org_short,
        'Top-Organisation': entry.org_top,
        'Notiz': entry.contact_note,
    ...
    // Autofilter für alle Sheets aktivieren
    ws1['!autofilter'] = { ref: "A1:J1" };
    ws2['!autofilter'] = { ref: "A1:J1" };
    ws3['!autofilter'] = { ref: "A1:J1" };
    ws4['!autofilter'] = { ref: "A1:J1" };


    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws1, 'Alle');
    XLSX.utils.book_append_sheet(wb, ws2, 'Ministerien');
    XLSX.utils.book_append_sheet(wb, ws3, 'Hochschulen');
    XLSX.utils.book_append_sheet(wb, ws4, 'Binnenmarkt');
...
function export_customers() {
    loadSheetJS(function () {
        ...
        $.get(url, function (response, status, xhr) {
            ...
            // Alle Kunden außer "Major Incident" filtern
            const customersData = allData.filter(entry =>
                entry.org_top !== "Major Incident"
            ).sort((a, b) => a.org_name.localeCompare(b.org_name));
            console.log(customersData);

            // Behörden unter Staatsverwaltung filtern
            const staatsverwaltungData = customersData.filter(entry =>
                entry.org_top === "Staatsverwaltung" || entry.customer_top === "Staatsverwaltung"
            );

            // Behörden unter Hochschule filtern
            const hochschuleData = customersData.filter(entry =>
                entry.org_top === "Hochschulen" || entry.customer_top === "Hochschulen"
            );

            // Organisationen mit org_binnenmarkt === true filtern
            const binnenmarktData = customersData.filter(entry =>
                entry.org_binnenmarkt === true
            );

            exportToExcel(customersData, staatsverwaltungData, hochschuleData, binnenmarktData);
